...
class DHCPLeaseHash(name: String,
                   elementType: String,
...
class DHCPLeaseHashElementBuilder(name: String, elementType: String)
  extends ElementBuilder(name, elementType) {

  override def buildElement: GenericElement = {
    new DHCPLeaseHash(name, elementType, getInputPorts, getOutputPorts, getConfigParameters)
  }
}

object DHCPLeaseHash {
  private var unnamedCount = 0

  private val genericElementName = "dhcpleasehash"

  private def increment {
    unnamedCount += 1
  }

  def getBuilder(name: String): DHCPLeaseHashElementBuilder = {
    increment ; new DHCPLeaseHashElementBuilder(name, "DHCPLeaseHash")
  }

  def getBuilder: DHCPLeaseHashElementBuilder =
