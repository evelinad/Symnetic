...
class DHCPServerRelease(name: String,
                   elementType: String,
...
class DHCPServerReleaseElementBuilder(name: String, elementType: String)
  extends ElementBuilder(name, elementType) {

  override def buildElement: GenericElement = {
    new DHCPServerRelease(name, elementType, getInputPorts, getOutputPorts, getConfigParameters)
  }
}

object DHCPServerRelease {
  private var unnamedCount = 0

  private val genericElementName = "dhcpserverrelease"

  private def increment {
    unnamedCount += 1
  }

  def getBuilder(name: String): DHCPServerReleaseElementBuilder = {
    increment ; new DHCPServerReleaseElementBuilder(name, "DHCPServerRelease")
  }

  def getBuilder: DHCPServerReleaseElementBuilder =
