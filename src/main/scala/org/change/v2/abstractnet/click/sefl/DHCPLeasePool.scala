...
class DHCPLeasePool(name: String,
                   elementType: String,
...
class DHCPLeasePoolElementBuilder(name: String, elementType: String)
  extends ElementBuilder(name, elementType) {

  override def buildElement: GenericElement = {
    new DHCPLeasePool(name, elementType, getInputPorts, getOutputPorts, getConfigParameters)
  }
}

object DHCPLeasePool {
  private var unnamedCount = 0

  private val genericElementName = "dhcpleasepool"

  private def increment {
    unnamedCount += 1
  }

  def getBuilder(name: String): DHCPLeasePoolElementBuilder = {
    increment ; new DHCPLeasePoolElementBuilder(name, "DHCPLeasePool")
  }

  def getBuilder: DHCPLeasePoolElementBuilder =
