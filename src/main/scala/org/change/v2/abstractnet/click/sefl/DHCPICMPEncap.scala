...
class DHCPICMPEncap(name: String,
                   elementType: String,
...
class DHCPICMPEncapElementBuilder(name: String, elementType: String)
  extends ElementBuilder(name, elementType) {

  override def buildElement: GenericElement = {
    new DHCPICMPEncap(name, elementType, getInputPorts, getOutputPorts, getConfigParameters)
  }
}

object DHCPICMPEncap {
  private var unnamedCount = 0

  private val genericElementName = "dhcpicmpencap"

  private def increment {
    unnamedCount += 1
  }

  def getBuilder(name: String): DHCPICMPEncapElementBuilder = {
    increment ; new DHCPICMPEncapElementBuilder(name, "DHCPICMPEncap")
  }

  def getBuilder: DHCPICMPEncapElementBuilder =
